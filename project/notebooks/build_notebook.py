...
"# Kadwanchi alone is ~97% agriculture+sparse-vegetation, so dense vegetation and",
"# barren/degraded land had almost no training signal. Both verified via a",
"# ground-truth class-distribution check before adding (ESA WorldCover clip + histogram):",
...
"            patch_id += 1",
"    print(f'{aoi_name} {date_tag}: {patch_id} base patches -> {len(records)} tiles')",
"    return records",
"",
"all_records = []",
"for job in AOI_JOBS:",
...
"        writer.writerow([fname, 'val' if fname in val_set else 'train'])",
"print(f'Total tiles: {len(all_records)}  (train={len(all_records)-n_val}, val={n_val})')",
...
"        print('NOTE: absent from val set (no ground truth to check):', ', '.join(metrics['classes_absent_from_val_set']))",
...
"",
"def ndvi_trend(ndvi_t1, ndvi_t2):",
...
"loss_fn2 = lambda logits, target: dice(logits, target) + ce(logits, target)",
...
"        best_val2 = val_loss",
"        torch.save({'model_state': model2.state_dict(), 'epoch': epoch, 'val_loss': val_loss}, ckpt_path2)",
"        print(f'  -> saved best checkpoint ({ckpt_path2.name})')",
"print(f'Done. Best val_loss={best_val2:.4f}. Checkpoint: {ckpt_path2}')",
))
...
"trend = ndvi_trend(img_t1[4], img_t2[4])",
"",
...
"    return torch.argmax(logits, dim=1).squeeze(0).cpu().numpy()[:orig_h,:orig_w].astype('uint8'), img, profile",
"",
...
"CLASS_HEALTH_WEIGHT = {WATER:100, DENSE_VEG:90, AGRI:70, SPARSE_VEG:55, FALLOW:35, BUILTUP:20, BARREN:10}",
"def compute_health_score(class_map):",
"    return float(np.mean(np.vectorize(CLASS_HEALTH_WEIGHT.get)(class_map)))",
"",
...
"health = compute_health_score(class_t2)",
"trend = float(np.mean(img_t2[4]) - np.mean(img_t1[4]))",
